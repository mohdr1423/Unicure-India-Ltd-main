...
import { listAdminRequests, decideAdminRequest, type AdminRequestRow } from "@/lib/admin-requests-manage.functions";
...
export const Route = createFileRoute("/_authenticated/admin/requests")({
  ...
  head: () => ({ meta: [{ title: "Admin requests — Unicure India Ltd" }, { name: "robots", content: "noindex" }] }),
});
...
function RequestsPage() {
  ...
  const mutate = useMutation({
    mutationFn: (vars: { requestId: string; decision: "approve" | "reject" }) =>
      decide({ data: vars }),
    onSuccess: (res) => {
      toast.success(res.status === "approved" ? "Request approved — user is now an admin." : "Request rejected.");
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Approve or reject users who have requested admin access to /admin/*.</p>
      </div>

      {isLoading && <p className="text-sm text-muted-foreground">Loading…</p>}
      {error && <p className="text-sm text-destructive">{error instanceof Error ? error.message : "Failed to load requests"}</p>}
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">History ({decided.length})</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          {decided.length === 0 ? (
...
function StatusBadge({ status }: { status: AdminRequestRow["status"] }) {
  if (status === "approved") return <Badge className="bg-green-600 hover:bg-green-600"><CheckCircle2 className="h-3 w-3 mr-1" />Approved</Badge>;
